...
var w = fInit();
// console.setTitle("学习减压");
// console.show();
fInfo("学习减压Pro" + newest_version + "脚本初始化");
// 初始化宽高
...
update_info = {
  ...
  "dati_tiku_link2": "https://mart-17684809426.coding.net/p/tiku/d/tiku/git/raw/master/dati_tiku_20220917.txt?download=false",
  "tiku_version": 20230410,
  "tiku_link": "https://gitcode.net/m0_64980826/songge_tiku/-/raw/master/tiku_json_20230410.txt",
  "tiku_link2": "https://mart-17684809426.coding.net/p/tiku/d/tiku/git/raw/master/tiku_json_20221117.txt?download=false",
...
// 设置资源保存路径
files.createWithDirs("/sdcard/学习减压/");
// 调整音量
if (yl_on) {
  fInfo("设置媒体音量");
...
function do_duizhan1(renshu) {
  ...
  while (true) {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (!que_txt) {
        images.save(img, '/sdcard/学习减压/' + renshu + '-' + num + '.png', 'png', 50);
        images.save(que_img, '/sdcard/学习减压/' + renshu + '-' + num + '-q.png', 'png', 50);
        fError("未识别出题目，图片保存至‘/sdcard/学习减压/’");
    ...
    // log(allx_txt);
    if (!allx_txt) {
      images.save(img, '/sdcard/学习减压/' + renshu + '-' + num + '-a.png', 'png', 50);
      log("识别不出选项文本，图片保存至‘/sdcard/学习减压/’");
      err_flag = false;
      sleep(200);
      continue;
    }
...
function init_wh() {
  ...
  if (device.width == device_h && device.height == device_w) {
    ...
    fError("识别不出设备宽高，建议重启'学习减压助手'后重新运行脚本");
    sleep(10000);
  }
...
function ocr_test() {
  ...
  try {
    ...
    if (test_time > test_limit) {
      ...
      fError("如偶然变慢，可能为无障碍服务抽风，建议重启'学习减压助手'后重试");
      sleep(3000);
      return false
    } else {
      fInfo("OCR功能正常");
...
function send_email(email) {
  ...
  data.putExtra(Intent.EXTRA_SUBJECT, "学习减压：" + name);
...
function fInit() {
  ...
  var w = floaty.rawWindow(
    <card cardCornerRadius='8dp' alpha="0.8">
      <vertical>
        ...
        <text id='version' textColor="#FFFFFF" textSize="18dip">学习减压+</text>
  ...
  ui.run(function () {
    ...
    w.version.setText("学习减压+" + newest_version);
  });
  w.setSize(720, -2);
  w.setPosition(10, 10);
...
function xxqg(userinfo) {
  ...
  d = 1;
  0 != dingyue && ("old" == jifen_flag && "0" == jifen_list.child(jifen_map["订阅"]).child(2).text().match(/\d+/)[0] || "old" == jifen_flag && "0" == jifen_list.child(jifen_map["订阅"]).child(3).child(0).text() || "old" == jifen_flag && "0" == jifen_list.child(jifen_map["订阅"]).child(3).text().match(/\d+/)[0]) && (toastLog("订阅开始"), d = do_dingyue(), jifen_list = refind_jifen());
  if (pushplus || token) {
    fInfo("推送前等待积分刷新5秒");
    sleep(5E3);
